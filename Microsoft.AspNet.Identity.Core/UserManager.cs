...
namespace Microsoft.AspNet.Identity
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        private IUserSaltStore<TUser, TKey> GetSaltStore()
        {
            var cast = Store as IUserSaltStore<TUser, TKey>;
            if (cast == null)
            {
                throw new NotSupportedException(Resources.StoreNotIUserSaltStore);
            }
            return cast;
        }
        ...
        public virtual async Task<IdentityResult> CreateAsync(TUser user, string password)
        {
            ...
            var result = await UpdatePassword(passwordStore, user, password, saltStore).WithCurrentCulture();
            if (!result.Succeeded)
            {
                return result;
            }
        ...
        public virtual async Task<bool> CheckPasswordAsync(TUser user, string password)
        {
            ...
            return await VerifyPasswordAsync(passwordStore, user, password, saltStore).WithCurrentCulture();
        }
            ...
            string newPassword)
        {
            ...
            if (await VerifyPasswordAsync(passwordStore, user, currentPassword, saltStore).WithCurrentCulture())
            {
                var result = await UpdatePassword(passwordStore, user, newPassword, saltStore).WithCurrentCulture();
                if (!result.Succeeded)
                {
                    return result;
                }
        ...
        protected virtual async Task<IdentityResult> UpdatePassword(IUserPasswordStore<TUser, TKey> passwordStore,
            TUser user, string newPassword, IUserSaltStore<TUser, TKey> saltStore)
        {
            ...
            if (!(await saltStore.HasSaltAsync(user)))
            {
                salt = await saltStore.GetSaltAsync();
            }
            else
            {
                salt = await saltStore.GetSaltAsync(user);
            }
            await
                passwordStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(newPassword, salt)).WithCurrentCulture();
            await UpdateSecurityStampInternal(user).WithCurrentCulture();
            return IdentityResult.Success;
        }
        ...
        protected virtual async Task<bool> VerifyPasswordAsync(IUserPasswordStore<TUser, TKey> store, TUser user,
            string password, IUserSaltStore<TUser, TKey> saltStore)
        {
            if (!await saltStore.HasSaltAsync(user))
                throw new ArgumentNullException("salt");
            string salt = await saltStore.GetSaltAsync(user);
            var hash = await store.GetPasswordHashAsync(user).WithCurrentCulture();
            return PasswordHasher.VerifyHashedPassword(hash, password, salt) != PasswordVerificationResult.Failed;
        }
