...
namespace Microsoft.AspNet.Identity
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        private async Task ValidateUserName(TUser user, List<string> errors)
        {
            ...
            else if (AllowOnlyAlphanumericUserNames && !Regex.IsMatch(user.user_name, @"^[A-Za-z0-9@_\.]+$"))
            {
                // If any characters are not letters or digits, its an illegal user name
                errors.Add(String.Format(CultureInfo.CurrentCulture, Resources.InvalidUserName, user.user_name));
            }
            else
            {
                var owner = await Manager.FindByNameAsync(user.user_name).WithCurrentCulture();
                if (owner != null && !EqualityComparer<TKey>.Default.Equals(owner.id, user.id))
                {
                    errors.Add(String.Format(CultureInfo.CurrentCulture, Resources.DuplicateName, user.user_name));
                }
            }
        }
